...
const GSTR2 = () => {
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await api.get(`/gst/gstr2?company=${companyId}&month=${month}&year=${year}`);
      setReport(data.data);
    } catch (err) {
      setError(err.response?.data?.message || err.message || 'Failed to load GSTR-2 report.');
      setReport(null);
    } finally {
      setLoading(false);
    }
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-cyan-50">
        <div className="container mx-auto px-6 py-8">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-10 animate-fade-in">
            <div>
              <h1 className="text-5xl font-bold gradient-text mb-2">GSTR-2 (Purchase)</h1>
              <p className="text-gray-600 text-lg">Monthly inward supplies summary</p>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-gray-100">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-gray-500">Registered vendors (B2B) with tax breakup</p>
              </div>
              <span className="text-sm text-gray-500">{entries.length} records</span>
            </div>
              ...
              <table className="w-full">
                <thead className="bg-gradient-to-r from-purple-600 to-pink-500 text-white">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-sm font-semibold uppercase tracking-wide">Type</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {entries.length === 0 && (
                    <tr>
                      <td colSpan="7" className="px-6 py-8 text-center text-gray-500">No purchase invoices for this period.</td>
                    </tr>
                  )}
                  {entries.map((entry) => (
